...
public class ActivityLogValidator extends AbstractValidator<ValidActivityLog, ActivityLog> {
	...
	public boolean isValid(final ActivityLog activityLog, final ConstraintValidatorContext context) {
		...
		if (activityLog == null)
			...
			super.state(context, legStarted, "leg", "acme.validation.activityLog.leg.not-finished");
			boolean legPublished = activityLog.getActivityLogAssignment().getLeg().isDraftMode();
			super.state(context, !legPublished, "leg", "acme.validation.flightAssignment.legNotPublished");
		}
