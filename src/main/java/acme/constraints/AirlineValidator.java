...
public class AirlineValidator extends AbstractValidator<ValidAirline, Airline> {
	...
	public boolean isValid(final Airline airline, final ConstraintValidatorContext context) {
		...
		if (airline == null) {
			...
			Airline exists = this.airlineRepository.findAllAirlineByIataCode(airline.getIataCode());
			boolean unique = exists == null || exists.equals(airline);
			super.state(context, unique, "iataCode", "acme.validation.airline.iataCode.unique");
